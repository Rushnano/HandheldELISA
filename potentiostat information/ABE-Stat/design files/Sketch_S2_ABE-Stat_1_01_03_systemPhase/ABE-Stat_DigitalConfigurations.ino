 ...
 */
 void TIA_AD5933() {
    ShiftRegisterValue |= 0x0030; // force O04 and O05 high (connect working electrode to LMP7721 TIA network)
    sh_reg(ShiftRegisterValue);
 }
...
/*
 * Conduct through PMOS to power AD5933 (if true), otherwise close channel / remove power
 */
 void AD5933_PowerOn(boolean powerStatus) {
    if (powerStatus) ShiftRegisterValue &= 0xefff;  // set bit 12 to 0 (pull inversion layer in PMOS to power network)
