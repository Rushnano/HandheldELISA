...
! This is a test program for UWED to make basic SWV recordings
...
! This parameter determines the SWV frequency
! SWV frequency = 1/(4*timestep), for example timeStep=25 --> 50ms sampling time,
! 100ms full SW cycle (--> SW frequency: 10Hz)
StepE=5				! [mV] Step size. Potential change in each SW cycle
pulseAmp=25			! [mV] SW amplitude
...
pulseAmpD=2*(pulseAmp/1000)*Rslope ! This is pulse amplitude, odd -1, even +1
DIM initW AS LONG
...
p1.setTitle("Electrochemical SWV with UWED") 
...
SUB InitializeScan()
	...
	IF SCi>nrScans THEN ! If increase scan counter is larger, DONE
	 sendToUWED("C(0)") ! Put UWED to potentiometric (off state)
		! Save data
		OPEN "SWVData.txt" FOR OUTPUT AS #1
		FOR i=1 TO DPi
		...
		em_mess="This is data from SWV measurement from iPhone"
		em=System.newEmail("email@emailserver.com","SWV Data", em_mess)
		em.addAttachment("SWVData.txt", "text/plain")
		em.send
		myMess.setText("DONE")
		ScanRunning=0
	ELSE ! Initialize a new scan
		...
		sendToUWED("G("&STR(0-pulseAmpD)&")")
		sendToUWED("I("&STR(stepR(SCi)+pulseAmpD)&")")
